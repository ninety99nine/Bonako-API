...
class UpdateCouponRequest extends FormRequest
{
    ...
    public function rules()
    {
        ...
        return [
            ...
            /*  General Information  */
            'name' => [
                'bail', 'sometimes', 'string', 'min:'.Coupon::NAME_MIN_CHARACTERS, 'max:'.Coupon::NAME_MAX_CHARACTERS,
                //  Make sure that this coupon name does not already exist for the same store
                Rule::unique('coupons')->where('store_id', request()->input('store_id'))->ignore(request()->couponId)
            ],
