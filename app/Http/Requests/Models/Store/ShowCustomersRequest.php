...
class ShowCustomersRequest extends FormRequest
{
    ...
    {
        $filters = collect(UserStoreAssociation::CUSTOMER_FILTERS)->map(fn($filter) => strtolower($filter));
    ...
    {
        return [
            'filter.string' => 'Answer "'.collect(UserStoreAssociation::CUSTOMER_FILTERS)->join('", "', '" or "').' to filter customers',
            'filter.in' => 'Answer "'.collect(UserStoreAssociation::CUSTOMER_FILTERS)->join('", "', '" or "').' to filter customers',
        ];
    }
