 ...
 *      });
 *
 *  (3) ...?sort={"defaultStore.orders.amount" : "desc"}
 *
 *      $query->with('defaultStore.orders', function (Builder $query) {
 *          //  Do the query stuff here
 *      });
 *
 ...
 *
 *  sort = {
 *      "defaultStore.orders.amount" : "desc"
 *  }
 */
class RepositorySorting
{
    ...
    public function runSortingQuery($model, $operation){
        ...
        if( $operation['direction'] == 'random' ) {
             ...
             *  $target = "stores.orders.amount"         (With relationships)
             */
